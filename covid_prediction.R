...
caret_methods = c( "krlsPoly", "rf", "enet", "bridge", "lars")
...
carrotify <- function(input_df, future_df, min_r2, max_r2, caret_methods) {
  ...
  #caret_methods = list of prediction methods
  #retuns a df of x, (predicted)y, algo
  #--------------------------------
  
  combined_df = tibble() #this will contain all the predicted values of all the algorithms
  caret_model_list = c()
  r2_list = c()
  for (method in caret_methods){
    ...
    skip_to_next=F
    tryCatch({caret_model = caret::train(y ~ x, data=input_df,method=method,trControl=tc , preProcess = c("scale", "center"))}, error = function(e){skip_to_next <<- TRUE})
    if (skip_to_next) {
      r2_list = c(r2_list, 0)
      next}
    ...
    caret_model_list = c(caret_model_list, caret_model$finalModel)
...
generate_single_prediction <- function(main_df, search_location){
  #load data csv
  df = main_df
  ...
  return(list(now=temp_now, past=temp_past)) 
  
}


df = read.csv(data_file)
dx = generate_single_prediction(df, "_US")
